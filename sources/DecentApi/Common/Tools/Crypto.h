...
namespace Decent
{
	namespace Tools
	{
		 ...
		 * \tparam	AddDataType	Type of the additional data.
		 * \param 	   	key			   	The key used for encryption.
		 * \param 	   	keyMeta		   	The key metadata (only MACed), which can be used to recover the
		 * 								key when decryption.
		 * \param 	   	meta		   	The metadata (encrypted), the secret metadata. (Will be copied,
		 * 								and then the internal temp var will be zeroized)
		 * \param 	   	data		   	The secret data. (Will be copied, and then the internal temp var
		 * 								will be zeroized)
		 * \param 	   	addData		   	The additional data used for authentication. (Will be copied, and
		 * 								then the internal temp var will be zeroized)
			...
			General128Tag& outTag, const size_t sealedBlockSize)
		{
			...
			return detail::QuickAesGcmPack(GetPtr(key), GetSize(key),
				...
				GetPtr(addData), GetSize(addData),
				outTag,
				sealedBlockSize);
		}
		 ...
		 * \param 	   	inTag		   	The input tag. If it's not null, it will be used to compare with
		 * 								the tag stored in the package.
		 * \param 	   	sealedBlockSize	The step size of the sealed block (must be the same as the one
		 * 								used in QuickAesGcmPack).
		 */
		template<typename KeyType, typename DataType, typename AddDataType>
		void QuickAesGcmUnpack(const KeyType& key, const DataType& encData, const AddDataType& addData,
				std::vector<uint8_t>& outMeta, std::vector<uint8_t>& outData,
