	...
	namespace Crypto
	{
		...
		public: // static members:
			...
			> GetTotalSealedBlockSize(
				size_t inSealedBlockSize, size_t inKeyMetaSize, size_t inMetaSize, size_t inDataSize
			)
			{
				const size_t totalDataSize = sk_sealPkgAllKnownSize + inKeyMetaSize + inMetaSize + inDataSize;
				...
				EXCEPTION_ASSERT(totalBlockSize == sizeof(TagType) + addSize + encryptedSize,
			...
			static std::vector<uint8_t> GetKeyMeta(const std::vector<uint8_t>& pack)
			{
				if (pack.size() < sk_sealMetaSize)
				{
					throw RuntimeException(
						"Decent::Crypto::AesGcmPacker::GetKeyMeta - "
		...
		public:
			...
			AesGcmPacker(const mbedTLScpp::ContCtnReadOnlyRef<_SecCtnType, true>& key,
			...
			AesGcmPacker(AesGcmPacker&& rhs) noexcept :
				m_gcm(std::move(rhs.m_gcm)), //noexcept
				m_sealedBlockSize(rhs.m_sealedBlockSize)
			{}

			AesGcmPacker(const AesGcmPacker& rhs) = delete;
			...
			std::pair<std::vector<uint8_t>, std::array<uint8_t, 16> > Pack(
			...
			)
			{
				...
				std::tie(totalPackSize, packAddSize, encryptSize) = GetTotalSealedBlockSize(
					m_sealedBlockSize, keyMeta.GetRegionSize(), meta.GetRegionSize(), data.GetRegionSize()
				);
...

				{
					...
					// Meta Size
					uint64_t& metaSize = *reinterpret_cast<uint64_t*>(inputPkg.data() + ipMetaSizePos);
					metaSize = meta.GetRegionSize();
					// Data Size
					uint64_t& dataSize = *reinterpret_cast<uint64_t*>(inputPkg.data() + ipDataSizePos);
					dataSize = data.GetRegionSize();
					// Meta
					//std::copy(meta.BeginBytePtr(), meta.EndBytePtr(), inputPkg.data() + ipMetaPos);
					std::memcpy(inputPkg.data() + ipMetaPos,
						meta.BeginBytePtr(), meta.GetRegionSize());
					// Data
					//std::copy(data.BeginBytePtr(), data.EndBytePtr(), inputPkg.data() + ipMetaPos + metaSize);
					std::memcpy(inputPkg.data() + ipMetaPos + metaSize,
						data.BeginBytePtr(), data.GetRegionSize());
				}
...

				{
					...
					// Generate IV
					DefaultRbg().Rand(finPackage.data() + fpIvPos, sizeof(IVType));
					// Payload Size
					uint64_t& payloadSize = *reinterpret_cast<uint64_t*>(finPackage.data() + fpPaySizePos);
					payloadSize = encryptSize;
					// Key Meta Size
					uint64_t& keyMetaSize = *reinterpret_cast<uint64_t*>(finPackage.data() + fpKMetaSizePos);
					keyMetaSize = keyMeta.GetRegionSize();
					// Key Meta
					std::memcpy(finPackage.data() + fpKMetaPos, keyMeta.BeginBytePtr(), keyMeta.GetRegionSize());
					...
					if (addData.GetRegionSize() > 0)
					{
						...
						pkgAddData.insert(pkgAddData.end(), finPackage.begin() + fpIvPos, finPackage.begin() + fpEncDataPos);
						pkgAddData.insert(pkgAddData.end(), addData.BeginBytePtr(), addData.EndBytePtr());

						// Encrypt
						std::tie(encrypted, tag) = m_gcm.Encrypt(
							CtnFullR(inputPkg),
					...
					EXCEPTION_ASSERT(finPackage.size() == (fpEncDataPos + encrypted.size()),
						"AesGcmPacker::Pack - The package size calculation result is wrong.");

					// Copy Tag
					std::memcpy(finPackage.data() + fpTagPos, tag.data(), tag.size());
					// Copy Encrypted Data
					std::memcpy(finPackage.data() + fpEncDataPos, encrypted.data(), encrypted.size());
				}
			...
			template<
				typename _DataCtnType, bool _DataCtnSecrecy,
				typename _AddCtnType,  bool _AddCtnSecrecy>
			std::pair<mbedTLScpp::SecretVector<uint8_t> /* Data */, mbedTLScpp::SecretVector<uint8_t> /* Meta */ > Unpack(
			...
			)
			{
				...
				SecretVector<uint8_t> outputPkg;
				{
					...
					const uint64_t payloadSize = *reinterpret_cast<const uint64_t*>(package.BeginBytePtr() + fpPaySizePos);
					const uint64_t keyMetaSize = *reinterpret_cast<const uint64_t*>(package.BeginBytePtr() + fpKMetaSizePos);

					const     size_t fpEncDataPos = fpKMetaPos + keyMetaSize;

					if (package.GetRegionSize() != fpEncDataPos + payloadSize)
					{
						throw RuntimeException("Decent::Crypto::AesGcmPacker::Unpack - "
					...
					if (addData.GetRegionSize() > 0)
					{
						// Build Full Add Data
						std::vector<uint8_t> pkgAddData;
						pkgAddData.reserve(sk_knownAddSize + keyMetaSize + addData.GetRegionSize());
						pkgAddData.insert(pkgAddData.end(), package.BeginBytePtr() + fpIvPos, package.BeginBytePtr() + fpEncDataPos);
						pkgAddData.insert(pkgAddData.end(), addData.BeginBytePtr(), addData.EndBytePtr());

						outputPkg = m_gcm.Decrypt(
					...
					if (inTag != nullptr)
					{
						...
						if (std::memcmp(package.BeginBytePtr() + fpTagPos, inTag->data(), sizeof(TagType)) != 0)
						{
							throw RuntimeException("Decent::Crypto::AesGcmPacker::Unpack - "
								"The tag/MAC contained in the message package doesn't match the given one.");
						}
					}
				}
				...
				SecretVector<uint8_t> outData;
				{
					...
					const uint64_t& metaSize = *reinterpret_cast<const uint64_t*>(outputPkg.data() + opMetaSizePos);
					const uint64_t& dataSize = *reinterpret_cast<const uint64_t*>(outputPkg.data() + opDataSizePos);

					if (outputPkg.size() != opMetaPos + metaSize + dataSize)
					{
						throw RuntimeException("Decent::Crypto::AesGcmPacker::Unpack - "
							"The encrypted payload package size doesn't match the expected size.");
					}
