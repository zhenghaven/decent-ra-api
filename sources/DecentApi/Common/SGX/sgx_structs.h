...
#pragma pack(push, 1)
	...
	typedef struct _sgx_ias_report_t
	{
		...
		report_timestamp_t      m_timestamp;     //Mandatory field (Parsed based on API) (chrono is not availble, couldn't find a better data type now).
		uint8_t                 m_version;       //Mandatory field (version of IAS, currently is only "3")
		uint8_t                 m_status;        //Mandatory field (enum IasQuoteStatus)
		uint8_t                 m_revoc_reason;  //Optional field (validated by m_status) (enum IasRevocReason)
		uint8_t                 m_pse_status;    //Optional field (validated by itself) (enum IasPseStatus)
