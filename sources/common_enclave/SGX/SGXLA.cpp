...
extern "C" sgx_status_t ecall_sgx_la_init(const char* peerId, int isInitiator)
{
	{
		std::lock_guard<std::mutex> mapLock(g_peerMapMutex);
		if (k_serversMap.find(peerId) != k_serversMap.cend())
		{
	...
	sgx_dh_session_role_t role = isInitiator ? SGX_DH_SESSION_INITIATOR : SGX_DH_SESSION_RESPONDER;

	enclaveRet = sgx_dh_init_session(role, &(peerCtx->m_session));
