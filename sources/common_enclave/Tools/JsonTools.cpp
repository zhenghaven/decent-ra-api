...
std::string Tools::Json2StyleString(const rapidjson::Value & inJson)
{
...
JSON_EDITION::Value& Tools::JsonCommonSetString(JSON_EDITION::JSON_DOCUMENT_TYPE& doc, JSON_EDITION::Value & root, const std::string & index, const std::string & inStr)
{
	if (!root.IsObject())
	{
		root.SetObject();
...
JSON_EDITION::Value& Tools::JsonCommonSetObject(JSON_EDITION::JSON_DOCUMENT_TYPE& doc, JSON_EDITION::Value & root, const std::string & index, JSON_EDITION::Value & inObj)
{
	if (!root.IsObject())
	{
		root.SetObject();
