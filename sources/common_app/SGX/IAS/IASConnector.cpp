...
int16_t IASConnector::GetRevocationList(const sgx_epid_group_id_t & gid, std::string & outRevcList) const
{
	return IASUtil::GetRevocationList(gid, outRevcList, m_certPath, m_keyPath);
}

int16_t IASConnector::GetQuoteReport(const std::string & jsonReqBody, std::string & outReport, std::string & outSign, std::string & outCert) const
{
